Linear projection is the operation that maps an input sequence $X$ into the query, key, and value matrices used by [2.1-Self_Attention](2.1-Self_Attention.md). Each projection is a learned matrix multiplication:
...
- These matrices $Q$, $K$, $V$ are then passed directly into [2.1-Self_Attention](2.1-Self_Attention.md).
